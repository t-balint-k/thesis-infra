...
CREATE UNIQUE INDEX uix1 ON securities (symbol, exchange, country) WHERE security_type in ('stocks', 'forex_pairs', 'funds', 'bonds', 'indices', 'etfs', 'commodities');
CREATE UNIQUE INDEX uix2 ON securities (symbol, exchange, currency_base, currency_quote) WHERE security_type = 'cryptocurrencies';
